...
/**
 * String resource for the various apps. Each index value from
 * {@link StringIndexConstants} has a corresponding String value for all
...
public class Strings implements StringIndexConstants {
     ...
     */
    private static final String[] ISO_639_LANGUAGE_CODES = { "en", "de", "es",
            "fr", };

    private static final Integer[] LANGUAGE_CONSTANTS = { LANG_ENGLISH,
            LANG_GERMAN, LANG_SPANISH, LANG_FRENCH, };
    ...
    /**
     * A hashtable that maps from ISO 639 country codes to Integer objects with
     * the corresponding LANG_xyz constant for that language.
     */
    ...
    /**
     * Create a new String object for the given language and fill it with the
     * String array.
     */
    ...
    /**
     * Determines an ISO 639 code of a language suitable for the environment in
     * which the JVM is currently running. First calls
     * {@link #determineIsoCodeFromDefaultLocale()}. If that yields null, the
     * ISO code for {@link #DEFAULT_LANGUAGE} is returned. So different from
     * {@link #determineIsoCodeFromDefaultLocale()} this method always returns a
     * non-null value.
     * 
     * @return String with ISO 639 code of a language that fits the JVM
     *         environment, or the default language as fallback solution
     */
    ...
    /**
     * Gets the String denoted by the argument index. This index must be one of
     * the int constants defined in {@link StringIndexConstants}.
     * 
     * @return String with given index in the current language
     * @throws IllegalArgumentException
     *             is not a valid index from {@link StringIndexConstants}
     */
    public String get(int index) {
        try {
            return data[index];
        } catch (ArrayIndexOutOfBoundsException aioobe) {
            throw new IllegalArgumentException("Not a valid String index: "
                    + index);
        }
    }

    /**
     * Returns the language of this object as one of the LANG_xyz constants of
     * this class.
     */
    ...
    public void set(Integer languageConstant, String[] values) {
        if (languageConstant == null || languageConstant.intValue() < 0
                || languageConstant.intValue() >= ISO_639_LANGUAGE_CODES.length) {
            throw new IllegalArgumentException(
                    "Not a valid language constant: " + languageConstant);
        }
        if (values == null || values.length < 1) {
            throw new IllegalArgumentException(
                    "The values array argument must be non-null and have at least one element.");
        }
